...
@section('topbar-action')
    @guest
        {{-- tombol kosong, login/register sudah ada di topbar atas untuk guest --}}
    @endguest
@endsection
...
<div class="card" style="margin-bottom:16px">
    <div style="padding:16px 20px">
        <form method="GET" action="{{ route('buku.index') }}" style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
            <input type="text" name="cari" value="{{ request('cari') }}" placeholder="Cari judul, pengarang, atau ISBN..."
        ...
        <div class="card" style="padding:18px;display:flex;flex-direction:column;gap:10px">
            <div>
                ...
                <div style="font-size:12px;color:#94a3b8;margin-top:4px;line-height:1.6">
                    {{ $buku->kota_terbit ? $buku->kota_terbit . ': ' : '' }}{{ $buku->penerbit }}, {{ $buku->tahun_terbit }}
                    @if($buku->edisi)<br>{{ $buku->edisi }}@if($buku->cetakan), {{ $buku->cetakan }}@endif @endif
                    @if($buku->jumlah_halaman)<br>{{ $buku->jumlah_halaman }} hlm. @if($buku->ukuran_tinggi); {{ $buku->ukuran_tinggi }}@endif @endif
                    @if($buku->isbn)<br>ISBN: {{ $buku->isbn }}@endif
                </div>
            </div>
